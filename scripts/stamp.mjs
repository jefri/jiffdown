// @ts-check
/**
 * stamp — cut a CalVer release of @davidsouther/jiffdown.
 *
 ...
 * collision-free without depending on the day of the week. (The first stamp off
 * a legacy semver like `1.0.0` starts the CalVer line at `<year>.<week>.0`.)
 *
 ...
 *   6. publishes the package to npm (`--access public`, scoped package).
 *
...
function main() {
  ...
  if (dryRun) {
    ...
    try {
      ...
      console.warn(`[dry-run] npm publish --dry-run reported: ${message}`);
    }
  } else {
...
// Run only when invoked directly (`node scripts/stamp.mjs`), not when the test
